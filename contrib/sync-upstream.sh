...
setup() {
    ...
    echo "Adding remote \"$REMOTE\" with URL git@github.com:m0hitkumar/secp256k1.git. Continue with y"
    read -r yn
    case $yn in
        [Yy]* ) ;;
        * ) exit 1;;
    esac
    git remote add "$REMOTE" git@github.com:m0hitkumar/secp256k1.git &> /dev/null
...
do
    PRNUM=$(git log -1 "$COMMIT" --pretty=format:%s | sed s/'Merge \(m0hitkumar\/secp256k1\)\?#\([0-9]*\).*'/'\2'/)
    TITLE="$TITLE $PRNUM,"
    BODY=$(printf "%s\n%s" "$BODY" "$(git log -1 "$COMMIT" --pretty=format:%s | sed s/'Merge \(m0hitkumar\/secp256k1\)\?#\([0-9]*\)'/'[m0hitkumar\/secp256k1#\2]'/)")
done
# Remove trailing ","
TITLE=${TITLE%?}
